...
local function COPY(old)
    local W32 = {}
    for k, v in pairs(old) do W32[k] = v end
...
local function AND(a, b)
    ...
    for i = 1, #a do
        ...
        if a[i] and b[i] then c[i] = true end
    end
...
local function OR(a, b)
    ...
    for i = 1, #a do
        -- only need to set true bits; other bits remain false
        if a[i] or b[i] then c[i] = true end
    end
...
local function OR3(a, b, c)
    ...
    for i = 1, #a do
        -- only need to set true bits; other bits remain false
        if a[i] or b[i] or c[i] then d[i] = true end
    end
...
local function asHEX(a)
    ...
    local i = 1
    while i < #a do
        local binary = (a[i + 3] and '1' or '0') .. (a[i + 2] and '1' or '0') ..
                           (a[i + 1] and '1' or '0') ..
                           (a[i + 0] and '1' or '0')
...
local function sha1(msg)
    ...
    local L64 = string.char(0) .. string.char(0) .. string.char(0) ..
    ...
    while chunk < chunks do
        ...
        for t = 0, 15 do
            W[t] = from_hex(string.format("%02x%02x%02x%02x",
                                          msg:byte(start, start + 3)))
            start = start + 4
        end
...
local function sha1_binary(msg) return hex_to_binary(sha1(msg)) end
...
local function hmac_sha1(key, text)
    ...
    if #key > blocksize then key = sha1_binary(key) end
...
local exports = {
    SHA1_HEX = function(key, dataStr)
        if assert_arg(1, key, "string") then return nil end
        if assert_arg(2, dataStr, "string") then return nil end
        return hmac_sha1(key, dataStr)
    end,
    SHA1_BIN = function(key, dataStr)
        if assert_arg(1, key, "string") then return nil end
        if assert_arg(2, dataStr, "string") then return nil end
        return hmac_sha1_binary(key, dataStr)
    end
}
SHA1 = exports
...
---? define a value for getting the date
_today_ = os.date("*t")

---? add hex helper to string namespace
local string = string
...
function MakeCallable(obj)
    local meta = {__call = function(self, ...) return self:new(...) or self end}
...
-- ─── URL HANDLING ───────────────────────────────────────────────────────────────
--
local function get_route(url)
    local route
    -- check for a route matching `/route?params`)
    -- check for a route like `/status`
    if not route then route = string.gsub(url, '^/', '') end
    -- else a route ending in `/`
    if not route then
...
local function urldata(url)
    local request = {}
    --- extract the requests route
    request.route = get_route(url)
    --- get the endpoint and decode params
    for k, v in url:gmatch '([^/=]+)%?(.+)' do
        request.endpoint = k
        local params = string.fromHex(v)
        if params then
...
local HTTPHandler = {}
local Secret
function HTTPHandler:new(baseurl, secret, keys)
    self.base_url = (baseurl or '/')
    self.Routes = {}
    Secret = _today_["year"] .. _today_["month"] .. _today_["day"] .. secret
    self.AuthorisedKeys = {}
    self:AddKeys(keys)
end

function HTTPHandler:AddKeys(keys)
    for k, v in ipairs(keys) do
        self.AuthorisedKeys[k] = SHA1.SHA1_HEX(Secret, v)
    end
end

---* Route Builder, creates a new route Handler from a url path and function.
---| the handler function you provide should return true/false and single response table,
...
function HTTPHandler:addRoute(route, handler, requireAuth)
    if not self.Routes[route] then
        self.Routes[route] = function(request, ...)
            local needAuth = requireAuth
            local routeHandler = handler
            if not needAuth then
                return routeHandler(request, ...)
            else
                local key = request.params["authKey"]
                local status, isAuthorised, authReason = self:isAuthorisedKey(key)
                if (not status) or (not isAuthorised) then
                    local Body = [[<div id="content">
                    <strong>Unauthorised</strong></br></hr>
                    <p>Reason ${reason}</p>
                    </div>]]
                    local Params = {reason = (authReason or "invalid authkey")}
                    return false, {403, Body}, Params
                else
                    return routeHandler(request, ...)
                end
            end
...
--- Check route exists
function HTTPHandler:hasRoute(route) if self.Routes[route] then return true end end

--- Validate given AuthKey
local function validateKey(key, Keys)
    local found = false
    local reason = "no valid key match"
    for _, v in ipairs(Keys) do
        if v == key then
            found = true
            reason = "authorised key match"
            break
        end
    end
    return found, reason
...
function HTTPHandler:isAuthorisedKey(key)
    local keys = self.AuthorisedKeys
    local Authorised, Reason = pcall(validateKey, (key or ""), keys)
    return Authorised, Reason
end

--- Main Handler
function HTTPHandler:Handle(url, ...)
    ...
    if Handler then
        local vars
        Handled, response, vars = Handler(request, ...)
        if Handled then
            if type(response) == "table" then
                response = JSON.stringify(response)
            end
            if type(vars) == "table" then
                response = string_expand(response, vars)
            end
            return true, response
        else
            return false, 'Failed to Handle Request'
        end
    end
end
g_http_middleware = {}
HTTPHandler = MakeCallable(HTTPHandler)

RegisterModule('HTTPHandler', HTTPHandler)
return HTTPHandler